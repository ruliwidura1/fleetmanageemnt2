...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		$('.icon-submit').addClass('fa-spin');
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/fleetmanagement/departure/"); ?>",
            "fnServerParams": function ( aoData ) {
				aoData.push(
					{ "name": "is_departure", "value": $("#fl_is_departure").val() },
			...
			"fnServerData"	    : function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						var url = '<?=base_url(); ?>api_admin/fleetmanagement/departure/detail/'+id;
						$.get(url).done(function(response){
                            if(response.status==200 || response.status=='200'){
							var dta = response.data;				
								$("#ieid").val(dta.id);
								$("#iecdate").val(dta.cdate);
								$("#ieb_driver_id").val(dta.b_driver_id);
								$("#iec_vehicle_id").val(dta.c_vehicle_id);
								$("#iejam_keluar").val(dta.jam_keluar);
								$("#iearea_tujuan").val(dta.area_tujuan);
								$("#iedestination").val(dta.destination);
								$("#ieis_departure").val(dta.is_departure);
...
$("#modal_tambah").on("shown.bs.modal",function(e){
	...
	$("#ftambah").on("submit",function(e){
		...
		var url = '<?=base_url('api_admin/fleetmanagement/departure/tambah/');?>';
		$.ajax({
			type: 'post',
			url: url,
			data: fd,
...
$("#fedit").on("submit",function(e){
	...
	$('.icon-submit').addClass('fa-spin');
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/fleetmanagement/departure/edit/"); ?>'+ieid;
	$.ajax({
...
$("#ahapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			...
			$('.icon-submit').addClass('fa-circle-o-notch');
			$('.icon-submit').addClass('fa-spin');
			var url = '<?=base_url('api_admin/fleetmanagement/departure/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				if(response.status == 200 || response.status == 200){
					setTimeout(function(){
...
$("#adetail").on("click",function(e){
	...
	$("#modal_option").modal("hide");
	setTimeout(function(){
		window.location = '<?=base_url_admin("fleetmanagement/departure/detail/"); ?>'+ieid;
	},333);
});