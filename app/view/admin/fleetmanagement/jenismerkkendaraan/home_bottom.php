...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/fleetmanagement/jenismerkkendaraan/"); ?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						$("#adetail").attr("href","<?=base_url_admin("fleetmanagement/jenismerkkendaraan/detail/")?>"+ieid);
						$("#aedit").attr("href","<?=base_url_admin("fleetmanagement/jenismerkkendaraan/edit/")?>"+ieid);
					});
	...
	$("#fl_do").on("click",function(e){
		...
		$.post('<?=base_url("api_admin/fleetmanagement/jenismerkkendaraan/statistik/")?>',fd).done(function(dt) {
			NProgress.done();
			if (dt.status == 200) {
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/fleetmanagement/jenismerkkendaraan/hapus/')?>'+ieid;
