...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		$('.icon-submit').addClass('fa-spin');
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/fleetmanagement/bahanbakar/"); ?>",
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						$.get(url).done(function(response){
							...
							var dta = response.data;				
								...
								$("#ietgl_beli").val(dta.tgl_beli);
								$("#iejenis").val(dta.jenis);
                $("#iekapasitas").val(dta.kapasitas);
                $("#iejumlah_beli").val(dta.jumlah_beli);
                $("#ieharga").val(dta.harga);
                $("#ietotal_harga").val(dta.total_harga);
...
$("#modal_tambah").on("shown.bs.modal",function(e){
	...
	$("#ftambah").on("submit",function(e){
		...
		var url = '<?=base_url('api_admin/fleetmanagement/bahanbakar/tambah/');?>';
		$.ajax({
			type: 'post',
			url: url,
			data: fd,
...
$("#fedit").on("submit",function(e){
	...
	$('.icon-submit').addClass('fa-spin');
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/fleetmanagement/bahanbakar/edit/"); ?>'+ieid;
	$.ajax({
...
$("#ahapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			...
			$('.icon-submit').addClass('fa-circle-o-notch');
			$('.icon-submit').addClass('fa-spin');
			var url = '<?=base_url('api_admin/fleetmanagement/bahanbakar/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				if(response.status == 200 || response.status == 200){
					setTimeout(function(){
...
$("#adetail").on("click",function(e){
	...
	$("#modal_option").modal("hide");
	setTimeout(function(){
		window.location = '<?=base_url_admin("fleetmanagement/bahanbakar/detail/"); ?>'+ieid;
	},333);
});
...
$("#itotal_harga").priceFormat({
	prefix: 'Rp',
	centsSeparator: ',',
	thousandsSeparator: '.',
	centsLimit: 0
});
$("#itotal_harga").on("blur",function(e){
	e.preventDefault();
	$("#ihtotal_harga").val(toPlainFloat($(this).val()));
});
...
$("#ietotal_harga").priceFormat({
	prefix: 'Rp',
	centsSeparator: ',',
	thousandsSeparator: '.',
	centsLimit: 0
});
$("#ietotal_harga").on("blur",function(e){
	e.preventDefault();
	$("#iehtotal_harga").val(toPlainFloat($(this).val()));
});