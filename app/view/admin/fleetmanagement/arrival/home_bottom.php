...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		$('.icon-submit').addClass('fa-spin');
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/fleetmanagement/arrival/"); ?>",
            "fnServerParams": function ( aoData ) {
				aoData.push(
					{ "name": "is_completed", "value": $("#fl_is_completed").val() },
			...
			"fnServerData"	    : function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						var url = '<?=base_url(); ?>api_admin/fleetmanagement/arrival/detail/'+id;
						$.get(url).done(function(response){
							...
							var dta = response.data;				
								...
								$("#iejam_masuk").val(dta.jam_masuk);
								$("#iedestination").val(dta.destination);
								$("#ieis_completed").val(dta.is_completed);
...
$("#modal_tambah").on("shown.bs.modal",function(e){
	...
	$("#ftambah").on("submit",function(e){
		...
		var url = '<?=base_url('api_admin/fleetmanagement/arrival/tambah/');?>';
		$.ajax({
			type: 'post',
			url: url,
			data: fd,
...
$("#fedit").on("submit",function(e){
	...
	$('.icon-submit').addClass('fa-spin');
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/fleetmanagement/arrival/edit/"); ?>'+ieid;
	$.ajax({
...
$("#ahapus").on("click",function(e){
	...
	if(id){
		...
		if(c){
			...
			$('.icon-submit').addClass('fa-circle-o-notch');
			$('.icon-submit').addClass('fa-spin');
			var url = '<?=base_url('api_admin/fleetmanagement/arrival/hapus/'); ?>'+id;
			$.get(url).done(function(response){
				if(response.status == 200 || response.status == 200){
					setTimeout(function(){
...
$("#adetail").on("click",function(e){
	...
	$("#modal_option").modal("hide");
	setTimeout(function(){
		window.location = '<?=base_url_admin("fleetmanagement/arrival/detail/"); ?>'+ieid;
	},333);
});